...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 transition-all duration-700">
      <div className={`mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-700 ${scrolled ? 'max-w-5xl mt-3' : 'max-w-7xl mt-0'}`}>
        <div className={`flex items-center justify-between transition-all duration-700 ${
          scrolled
            ? 'rounded-2xl px-5 liquid-glass-deep'
        ...
        >
          <Link to="/" className="flex items-center gap-2.5 shrink-0">
            <motion.span
              className="w-7 h-7 rounded-lg bg-gradient-to-br from-[#60a5fa] to-[#f472b6] flex items-center justify-center text-[10px] font-extrabold text-white"
              whileHover={{ scale: 1.06, rotate: -5 }}
            >
              JC
            </motion.span>
            <motion.span
              className="text-base font-bold gradient-text-mixed"
          ...
          <div className="hidden md:flex items-center gap-0.5">
            {links.map((link) => (
              ...
              >
                {({ isActive }) => (
                  <>
                    <span className={`relative z-10 transition-colors duration-300 ${isActive ? 'text-white' : 'text-[var(--color-text-muted)] hover:text-white'}`}>
                      {link.label}
                    </span>
                    {isActive && (
                      <motion.div
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            <Link to="/contact">
              <motion.span
                className="px-3 py-1.5 rounded-lg text-[11px] font-semibold bg-[#4f8fde] text-white cursor-pointer hover:bg-[#60a5fa] transition-all"
                whileHover={{ scale: 1.02 }}
              >
                Hire Me
              </motion.span>
