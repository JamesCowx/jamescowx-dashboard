...
export default function Navbar() {
  ...
  return (
    <motion.nav className="fixed top-0 left-0 right-0 z-50 transition-all duration-500"
      animate={{ y: 0 }} initial={{ y: -100 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}>
      <div className={`mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-500 ${scrolled ? 'max-w-5xl mt-3' : 'max-w-7xl mt-0'}`}>
        <div className={`flex items-center justify-between transition-all duration-500 ${scrolled ? 'rounded-2xl px-5 bg-white/[0.04] backdrop-blur-2xl border border-white/[0.06] shadow-2xl' : 'rounded-none px-0 bg-transparent'}`}
          style={{ height: scrolled ? 56 : 68 }}>
          
          <Link to="/" className="flex items-center gap-2.5 shrink-0 group">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-[#60a5fa] to-[#f472b6] flex items-center justify-center text-[10px] font-extrabold text-white">
              JC
            </div>
            <span className="text-base font-bold text-white/90 group-hover:text-white transition-colors hidden sm:block">
              James Cowx
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-0.5">
            {links.map(link => (
              <NavLink key={link.to} to={link.to} end={link.to === '/'}
                ...
                {({ isActive }) => (
                  <>
                    <span className={`relative z-10 ${isActive ? 'text-white' : 'text-[var(--color-text-muted)] hover:text-white/80'}`}>{link.label}</span>
                    {isActive && <motion.div layoutId="nav-pill" className="absolute inset-0 rounded-xl bg-white/[0.06] border border-white/[0.08]" transition={{ type: 'spring', stiffness: 400, damping: 32 }} />}
                  </>
                )}
              </NavLink>
            ))}
